...
public class controllerlistemedadmin implements Initializable {
    ...
    public void showListemedadmin() {
        ...
        try {
            ...
            while (result.next()) {
                data.add(new modellistemedadmin(result.getInt("numero"), result.getString("nom"),
                        result.getString("type"), result.getString("dosage"), result.getDate("dateexpiration"),
                        result.getDate("datefabrication"), result.getFloat("prix")));
            }
    ...
    public void ajoutermed() {
        fxml = FXMLLoader.load(getClass().getResource("/fxml/ajoutermedicaments.fxml"));
